...
#k折交叉验证  测试集
accuracy = ms.cross_val_score(LR_model, x, y, cv=10, scoring='accuracy')
print('CV Accuracy:', accuracy.mean(), accuracy.std())
pw = ms.cross_val_score(LR_model, x, y, cv=10, scoring='precision_weighted')
print('CV Specificity:', pw.mean(), pw.std())
rw = ms.cross_val_score(LR_model, x, y, cv=10, scoring='recall_weighted')
print('CV Sensitivity:', rw.mean(), rw.std())
fw = ms.cross_val_score(LR_model, x, y, cv=10, scoring='f1_weighted')
print('CV F1-score:', fw.mean(), fw.std())
roc_auc = ms.cross_val_score(LR_model, x, y, cv=10, scoring='roc_auc')
...
plt.title("Ten-fold cross-validated ROC curve of LR",fontsize=10,fontproperties="Times New Roman")
